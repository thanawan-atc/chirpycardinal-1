...
class AskWhyFavoriteColorTreelet(Treelet):
    name = "ask_why_favorite_color_treelet"

    def get_response(self, priority=ResponsePriority.STRONG_CONTINUE):
        state, utterance, response_types = self.get_state_utterance_response_types()

        return ResponseGeneratorResult(
            text="I like it too! Do you have any specific reasons?",
            ...
            conditional_state=self.rg.ConditionalState(
                ...
                next_treelet_str=self.rg.comment_color_treelet.name
            )
        )
