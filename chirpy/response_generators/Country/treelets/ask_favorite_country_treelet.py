...
class AskFavoriteCountryTreelet(Treelet):
    name = "ask_favorite_country_treelet"

    def get_response(self, priority=ResponsePriority.STRONG_CONTINUE, **kwargs):
        """ Returns the response.
        :param **kwargs:
        """
        return ResponseGeneratorResult(text="Sure! What's your favorite country?", priority=ResponsePriority.CAN_START,
                                       needs_prompt=False, state=State(),
                                       cur_entity=None,
                                       answer_type=AnswerType.QUESTION_SELFHANDLING,
                                       conditional_state=ConditionalState(
                                           next_treelet_str="ask_why_favorite_country_treelet",
                                           cur_country=None),
                                       expected_type=ENTITY_GROUPS_FOR_EXPECTED_TYPE.country_related
                                       )