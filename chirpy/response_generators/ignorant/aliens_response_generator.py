...
from chirpy.response_generators.ignorant.treelets import *
from chirpy.response_generators.ignorant.aliens_helpers import *
from chirpy.response_generators.ignorant.state import State, ConditionalState
from chirpy.response_generators.ignorant.aliens_helpers import ResponseType

logger = logging.getLogger('chirpylogger')

SPACE_ENTITIES = ["Outer space", "Haumea", "Extraterrestrial life", "Alien (law)", "Planet"]

class IgnorantResponseGenerator(ResponseGenerator):
    name = "IGNORANT"
    def __init__(self, state_manager) -> None:
        ...
        treelets = {
            treelet.name: treelet for treelet in [self.introductory_treelet, self.first_turn_treelet,
                                                  self.second_turn_treelet, self.third_turn_treelet,
                                                  self.fourth_turn_treelet, self.fifth_turn_treelet,
                                                  self.question_treelet]
        }
    ...
    def handle_user_attributes(self):
        if self.state.prev_treelet_str == self.first_turn_treelet.name:
