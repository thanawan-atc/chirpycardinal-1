...
class CommentOnFavoriteTypeTreelet(Treelet):
    ...
    def get_prompt(self, conditional_state=None):
        ...
        elif is_subclassable(cur_animal.name.lower()):
            text = f"What type of {cur_animal.talkable_name} do you like the most?"
        else:
            return None
    ...
    def get_best_candidate_user_entity(self, utterance, cur_animal):
        def condition_fn(entity_linker_result, linked_span, entity):
            return EntityGroupsForExpectedType.animal_related.matches(entity) and entity.name != cur_animal
    ...
    def get_response(self, priority=ResponsePriority.STRONG_CONTINUE, **kwargs):
        ...
        cur_animal_entity = state.cur_animal
        cur_animal = cur_animal_entity.name
        cur_talkable_animal = cur_animal_entity.talkable_name

        user_answer, is_plural = self.get_best_candidate_user_entity(utterance, cur_animal)
        copula = infl('are', is_plural)
        pronoun = infl('they', is_plural)

        if get_custom_question(cur_animal) is not None:
            # try to find a animal first, but if that fails, just select the top entity.
            ...
            custom_question_answer = get_custom_question_answer(cur_animal)
            text = f"{response} Personally, when it comes to {cur_talkable_animal}, I really like {custom_question_answer}."
        else:
            # sample another of the same type
            entity = self.rg.state_manager.current_state.entity_tracker.cur_entity
            other_type = sample_from_type(cur_animal)
